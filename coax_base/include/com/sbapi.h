...
/** This callback will be sent if a repeated message is received. This means
 * that this message has been received on the second communication channel of
 * the micro-controller. This could be used to communicate between the gumstix
 * and the ground station using Xbee. However, a very limited bandwidth would
 * be available
 * */
...
/**
 * Send a message to be repeated by the micro controller to the other comm
 * channel. For instance from Xbee to serial port. See also sbRegisterRepeatCallback 
 * */
...
/**   Configure debug flags for the controller. To be extended
 *    when necessary. For now, the only accepted value is SB_DEBUG_ZERO_PROPELLER
 *    which will prevent the blades from rotating.
 *    */
int sbSetDebugMode(struct SBControlContext *control, unsigned char mode);
