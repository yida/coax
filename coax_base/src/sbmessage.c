...
#ifndef PIC30
int sbSetDebugEncode(SBSerialisedMessage *msg, unsigned char handle, 
		const SBSetDebugMode* state, int requestAck)
{
	unsigned char *m = msg->data+SB_MESSAGE_HEADER_LEN;
	sbInitializeMessage(msg,SB_MSGID_SET_DEBUG | (requestAck?SB_MSGID_REQACK:0), handle);
	m[0] = state->debugMode;
	msg->len = 1;
	return sbFinalizeMessage(msg);
}
...
int sbSetDebugDecode(const SBSerialisedMessage *msg, SBSetDebugMode* state)
{
	const unsigned char *m = msg->data+SB_MESSAGE_HEADER_LEN;
	CHECK_DEC(sbSetDebugDecode,SB_MSGID_SET_DEBUG,1);
	state->debugMode = m[0];
	return 0;
}
...
{
	const unsigned char *m = msg->data+SB_MESSAGE_HEADER_LEN;
	CHECK_DEC(sbCoaxSpeedSetLightDecode,SB_MSGID_SET_LIGHT,1);
