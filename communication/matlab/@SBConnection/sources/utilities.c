...
mxArray * convertStateToMex(const SBHeliState * state)
{
	...
	if (TESTCONTENT(state->content,SBS_O_ATTITUDE)) {
		...
		mxSetField(result,0,"o_attitude", mxCreateDoubleVect(3,state->o_attitude));
	}
	if (TESTCONTENT(state->content,SBS_O_ALTITUDE)) {
		mxAddField(result,"o_altitude");
		mxSetField(result,0,"o_altitude", mxCreateDoubleVect(1,&state->o_altitude));
	}
	...
	if (TESTCONTENT(state->content,SBS_O_XY)) {
		...
		mxSetField(result,0,"o_xy", mxCreateDoubleVect(2,state->o_xy));
	}
	if (TESTCONTENT(state->content,SBS_O_OAVOID)) {
		mxAddField(result,"o_oavoid");
		mxSetField(result,0,"o_oavoid", mxCreateDoubleVect(2,state->o_oavoid));
	}
	if (TESTCONTENT(state->content,SBS_CHANNELS)) {
		mxAddField(result,"rcChannel");
		mxSetField(result,0,"rcChannel", mxCreateDoubleVect(8,state->rcChannel));
	}

	if (TESTCONTENT(state->content,SBS_COAXSPEED)) {
		tmp =  mxCreateStructArray(1,dims,0,NULL);
        mxAddField(tmp,"state");
		mxSetField(tmp,0,"state", mxCreateScalarUChar(state->coaxspeed.state));
        mxAddField(tmp,"light");
		mxSetField(tmp,0,"light", mxCreateScalarUChar(state->coaxspeed.light));
        mxAddField(tmp,"vel_x");
		mxSetField(tmp,0,"vel_x", mxCreateDoubleScalar(state->coaxspeed.vel_x));
        mxAddField(tmp,"vel_y");
		mxSetField(tmp,0,"vel_y", mxCreateDoubleScalar(state->coaxspeed.vel_y));
		mxAddField(result,"coaxspeed");
		mxSetField(result,0,"coaxspeed", tmp);
	}
	return result;
}
...
#define TEST_AND_ADD(m) mxGetString(Pi,name,255); \
	if (stricmp(name,#m)==0) {value = value | SBS_##m;}
			...
			TEST_AND_ADD(CHANNELS);
			TEST_AND_ADD(COAXSPEED);
		}
	} else {
