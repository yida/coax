...
{
	int res;
	memset(&control->channel, 0, sizeof(SBChannel));
...
{
	int res;
	bzero(&control->channel, sizeof(SBChannel));
	...
	while (numtries > 0) {
		...
		if (reply->msgid == (SB_MSGID_REPLY|SB_MSGID_STATE)) {
			...
			if (control->stateCallback) {
				...
				control->stateCallback(&hs,control->stateUserData);
			}
        } else if (reply->msgid == SB_MSGID_REPEAT) {
#ifdef DEBUG_SEND_AND_WAIT
			printf("That's a repeat message\n");
#endif
			numtries ++; // that does not count :)
			if (control->repeatCallback) {
				control->repeatCallback(reply->data+SB_MESSAGE_HEADER_LEN,
                        reply->len,control->repeatUserData);
			}
	...
	void *userData)
{
	...
	control->stateUserData = userData;
	return 0;
}

int sbRegisterRepeatCallback(struct SBControlContext *control, 
	void (*repeatCallback)(const unsigned char*data, unsigned int len, void *userData),
	void *userData)
{
	control->repeatCallback = repeatCallback;
	control->repeatUserData = userData;
	return 0;
}
...
// debugMode defined from sbconst.h SB_DEBUG_xxxx
int sbSetDebugMode(struct SBControlContext *control, unsigned char debugMode)
{
	int r;
	SBSerialisedMessage sm,reply;
	SBSetDebugMode sd;
	SBBasicReplyMessage br;
	sd.debugMode = debugMode;
	sbSetDebugEncode(&sm,control->handle,&sd,control->ackMode);
...
int sbReceiveDataWithCallbacks(struct SBControlContext *control,
        unsigned int timeout_wait_ms, unsigned int timeout_receive_ms)
{
	SBHeliState state;
    SBSerialisedMessage reply;
    int res = -1;
    sbLockCommunication(control);
    // Receive state
    res = sbChannelWaitData(&control->channel, timeout_wait_ms); 
    if (res==0) {
        ...
        if (sbWaitRawMessage(&control->channel,-1,&reply,timeout_receive_ms)) {
            printf("Error while receiving raw message\n");
        } else {
    ...
    if (res==0) {
        if (reply.msgid == (SB_MSGID_REPLY|SB_MSGID_STATE)) {
            ...
            if (control->stateCallback) {
                // printf("And we have a callback\n");
                SBHeliStateRaw hr;
                sbStateDecode(&reply,&hr);
                scaleHeliState(&state,&hr);
                control->stateCallback(&state,control->stateUserData);
            }
        } else if ((reply.msgid & SB_MSGID_MASK)  == SB_MSGID_REPEAT) {
            // printf("\nThis is a repeat\n");
            if (control->repeatCallback) {
                // printf("And we have a callback\n");
                control->repeatCallback(reply.data+SB_MESSAGE_HEADER_LEN,
...
int sbSendRepeatMessage(struct SBControlContext *control, 
        const unsigned char *data, unsigned int len) {
	int i,r;
	SBSerialisedMessage sm,reply;
	SBBasicReplyMessage br;
    if (len > SB_STRING_MESSAGE_LENGTH) {
        printf("sbSendRepeatMessage: message too long\n");
        return -1;
    }
	sm.len = len;
	sm.handle = control->handle;
	sm.msgid = SB_MSGID_REPEAT;
	if (control->ackMode) {
		sm.msgid |= SB_MSGID_REQACK;
	}
    for (i=0;i<len;i++) {
        sm.data[SB_MESSAGE_HEADER_LEN+i] = data[i];
    }
	sbFinalizeMessage(&sm);
